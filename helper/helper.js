...
var helper = {
  ...
  leerDelFicheroLog: function (dato) {
    ...
    try {
      ...
      this.logError('No existe el fichero de log para los casos, la ruta esperada es: "' + path_log_pruebas + '"');
    ...
    const posicionFinal =
      posicionInicial + dato.length + contenido_fichero.substring(posicionInicial + dato.length).indexOf('-');
  ...
  reemplazarEnFichero: function (caso, estadoAnterior, estadoActual, estadoFuturo) {
    ...
    if (!noHayCasoDeLog) {
      ...
      const contenidoAnterior = contenido_fichero.substring(posicionInicial, posicionInicial + datos.length);
